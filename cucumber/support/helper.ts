...
import { existsSync, readFileSync, unlinkSync, writeFileSync, rmdirSync } from 'fs';
...
export const createTestFile = (name: string): void => {
  writeFileSync(
    name,
    `import { testControllerHolder } from "./cucumber/support/test-controller-holder";
...
export const addMetadata = (key: string, value: string): void => {
  ...
  json[key] = value;
  writeFileSync('cucumber/reports/metadata.json', JSON.stringify(json));
};
...
export const generateHtmlReport = (): void => {
  if (GENERATE_CUCUMBER_HTML_REPORT) {
    try {
      ...
      execSync(`node ${process.cwd()}/cucumber/cucumber-html.config.js`);
    } catch (error) {
...
export const generateJunitReport = (): void => {
  if (GENERATE_CUCUMBER_JUNIT_REPORT) {
    try {
      ...
      execSync(`node ${process.cwd()}/cucumber/cucumber-junit.config.js`);
    } catch (error) {
...
export const createTestFailFile = (): void => {
  ...
  writeFileSync(`cucumber/reports/${TEST_FAIL_FILE}`, '');
};
...
export const deleteSerenityReportFolder = (): void => {
  if (existsSync(SERENITY_REPORT_DIR)) {
    logger.info('Delete serenity report dir...');
    rmdirSync(SERENITY_REPORT_DIR, { recursive: true });
  }
};
