...
import {
  ...
  TEST_FAIL_FILE,
  VIDEO_DIR,
  TEST_SHOULD_START_APP,
  TEST_APP_START_DELAY,
  TEST_APP_START_COMMAND
} from '../environment';
...
function createServerAndRunTests(): void {
  createTestCafe(TEST_CAFE_HOST)
    .then((tc: TestCafe) => {
      ...
      if (TEST_SHOULD_START_APP) {
        runner = runner
          .startApp(TEST_APP_START_COMMAND, Number.parseInt(TEST_APP_START_DELAY));
      }
